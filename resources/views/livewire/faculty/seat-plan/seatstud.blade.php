...
<body class="font-body">
    ...
    <div class="flex flex-col items-center justify-center mt-6">
        ...
        <div class="font-medium text-1xl">SUBJECT: <span class="font-bold">{{ $subject ?? ' ' }}</span></div>
        <div class="font-medium text-1xl">SECTION: <span class="font-bold">{{ $section ?? ' ' }}</span></div>
        <div class="font-medium text-1xl">FACULTY: <span class="font-bold">{{ $faculty ?? ' ' }}</span></div>
    </div>
    ...
    <div class="flex items-center justify-center">
        <div class="grid grid-cols-2 gap-2 p-10 text-white">
                ...
                <div class="flex justify-center">
                    @foreach ($row as $seat)
                        ...
                        @elseif (!isset($seatplansArray[$seat->seat_number]))
                            <div id="{{ $seat->seat_number }}" class="flex items-center justify-center font-medium bg-blue-700 border border-black box" style="width: 40px; height: 40px; margin: 5px;">{{ $seat->seat_number }}</div>
                        @else
                            <div id="{{ $seat->seat_number }}" class="flex items-center justify-center font-medium bg-red-700 border border-black box" style="width: 40px; height: 40px; margin: 5px;">{{ $seat->seat_number }}</div>
                        @endif
                    @endforeach
                </div>
            @endforeach
            ...
            <div class="flex justify-center col-span-2 mt-5">
                <div class="flex items-center justify-center font-bold bg-blue-700 border border-black" style="width: 300px; height: 40px;">FACULTY TABLE</div>
            </div>
        </div>
    </div>
    ...
    <div class="flex gap-20 pl-10 pr-10">
        <div class="w-1/2">
            <table class="table table-zebra">
                <thead>
                    <tr class="rounded-md bg-base-200">
                        <th>SEAT</th>
                        <th>NAME</th>
                    </tr>
                </thead>
