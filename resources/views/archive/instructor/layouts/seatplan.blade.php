<!DOCTYPE html>
<html data-theme="{{ Auth::user()->instructor_theme ?? 'light' }}" lang="{{ str_replace('_', '-', app()->getLocale()) }}">
...
<head>
    ...
    <title> {{ $title ?? '' }} - {{ $appSetting->website_name ?? 'Pi:Lock | Instructor' }} </title>
        ...
        <div class="drawer-content bg-base-200">
            ...
            <nav class="navbar bg-base-100 sticky top-0 z-30">
                <div data-sveltekit-preload-data class="bg-base-100 sticky top-0 z-20 hidden items-center gap-2 bg-opacity-90 px-4 py-2 backdrop-blur lg:flex ">
                    <!-- Drawer Logo -->
                    <a href="{{ route('archive.instructor.dashboard.index') }}" aria-current="page" aria-label="Homepage" class="flex-0 btn btn-ghost px-2"
                ...
                <div class="flex-none gap-1">
                    ...
                    <div class="dropdown dropdown-end">
                        ...
                        <ul tabindex="0" class="mt-3 z-[1] p-2 shadow menu menu-sm dropdown-content bg-base-100 rounded-box w-52">
                            <li><a href="{{ route('archive.instructor.logout') }}"
                                    onclick="event.preventDefault(); document.getElementById('logout-form').submit();">{{ __('Logout') }}</a>
                                <form id="logout-form" action="{{ route('archive.instructor.logout') }}" method="POST" class="d-none">
                                    @csrf
                                </form>
                            </li>
                        </ul>
